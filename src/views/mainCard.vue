<template>
  <div class="row" v-show="mainCard">
    <div class="card card-profile" style="max-width: 500px">
      <div
        v-show="avatar"
        class="card-avatar wow fadeInDown"
      ...
      <div class="card-body">
        <div @click="showUserInfoIfLogined()">
          <h4
            class="card-title wow fadeInUp"
            data-wow-duration="2s"
            data-wow-delay="1s"
          >{{ mainCardName }}</h4>
          <h6
            class="card-category text-gray wow fadeInUp"
            data-wow-duration="2s"
            data-wow-delay="1s"
          >{{ mainСardSpecialization }}</h6>
        </div>

        <template v-for="userSocButton in userInfoSoc">
          <a
            class="btn btn-just-icon btn-link wow fadeInUp"
            data-wow-duration="2s"
        ...
        <div @click="showUserInfoIfLogined()">
          <p
            class="card-description wow fadeIn"
            data-wow-duration="2s"
            data-wow-delay="1s"
          >{{ mainCardAbout }}</p>
        </div>
        ...
        <div id="accordion" role="tablist" aria-multiselectable="false" class="card-collapse">
          <template v-for="userService in userServices">
            <div
              class="card card-plain wow fadeInUp"
              data-wow-duration="1s"
              :data-wow-delay="userService.delay"
              :key="userService"
            >
              <div
                class="card-header"
                role="tab"
                :id="userService.heading"
              ...
              <div
                :id="userService.collapse"
                class="collapse"
                role="tabpanel"
                :aria-labelledby="userService.heading"
              >
                        ...
                        <template v-if="ifLogined">
                          ...
                          <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <a
                              class="dropdown-item"
                              href="#"
                              @click="showAddServiceForm(userService.id)"
                            >Редактировать</a>
                            <a
                              class="dropdown-item"
                              href="#"
                              @click="deleteService(userService.id)"
                            >Удалить</a>
                          </div>
                        </template>
                      </div>
        ...
        .then(response => {
          if (response.data.userInfoName) {
            ...
            document.getElementById("backgroundDiv").style.backgroundImage =
              "url('img/backgrounds/" + response.data.background + "')";
            // другие переменные
            ...
            if (response.data.userLinkButtonAction === "notShow") {
